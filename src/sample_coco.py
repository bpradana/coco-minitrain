...
def main(args=None):
    ...
    parser.add_argument('--save_file_name', help='Save file name',
                        default="instances_train2017_minicoco")
    parser.add_argument(
        '--save_format', help='Save to json or csv', default="json")
    parser.add_argument('--sample_image_count',
                        help='How many images you want to sample', default=25_000)
    parser.add_argument(
        '--run_count', help='How many times you want to run sampling', type=int, default=10_000_000)
    parser.add_argument('--debug', help='Print useful info',
                        action='store_true')
    ...
    for rr in range(int(parser.run_count)):
        imgs = {}
        ...
        for k, v in imgs_best_sample.items():
            ...
            for ann in v:
                ...
                write_str = f_name + ',' + str(bbox[0]) + ',' + str(bbox[1]) + ',' + str(bbox[2]) + ',' + str(
                    bbox[3]) + ',' + \
                    str(dataset_train.labels[dataset_train.coco_labels_inverse[class_id]]
                        ) + ',' + '0' + '\n'
